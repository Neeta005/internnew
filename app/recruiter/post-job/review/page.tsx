...
export default function JobReviewPage() {
  ...
  // Mock data - in real app this would come from form state/database
  const jobData = {
...
Server Infrastructure/Setting up and maintaining the servers and infrastructure required to host the application.`,
  ...
  const handleNext = () => {
    ...
    if (activeTab === "basic") {
      console.log("[v0] Switching to roles tab")
      setActiveTab("roles")
    } else if (activeTab === "roles") {
      console.log("[v0] Switching to offer tab")
      setActiveTab("offer")
    } else if (activeTab === "offer") {
      console.log("[v0] Switching to questions tab")
      setActiveTab("questions")
    } else {
      console.log("[v0] Final step - navigating to dashboard")
      // Final step - navigate to success page or dashboard
      window.location.href = "/recruiter/dashboard"
    }
  }

  const handleTabClick = (tab: string) => {
    console.log("[v0] Tab clicked:", tab)
    setActiveTab(tab)
  }
      ...
      <div className={`flex-1 p-6 transition-all duration-300 ${sidebarCollapsed ? "ml-16" : "ml-64"}`}>
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex gap-3">
            ...
            <Button className="bg-red-500 hover:bg-red-600" onClick={handleNext}>
        ...
        {activeTab === "roles" && (
          <div className="space-y-6">
            ...
            <div>
              ...
              <div className="text-gray-300 leading-relaxed whitespace-pre-line">{jobData.frontEndDevelopment}</div>
            </div>

            <div>
              <h3 className="text-orange-500 font-medium mb-3">Back-End Development:</h3>
              <div className="text-gray-300 leading-relaxed whitespace-pre-line">{jobData.backEndDevelopment}</div>
            </div>
          </div>
        )}
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <Switch checked={jobData.trainingProvided} className="data-[state=checked]:bg-red-500" />
              </div>
              <div className="flex items-center justify-between">
                <span className="text-white">Offer Letter will be provided</span>
                <Switch checked={jobData.offerLetterProvided} className="data-[state=checked]:bg-red-500" />
              </div>
              <div className="flex items-center justify-between">
                <span className="text-white">Experience certificate will be provided</span>
                <Switch checked={jobData.experienceCertificateProvided} className="data-[state=checked]:bg-red-500" />
              </div>
            </div>

            {/* Stipend Section */}
            <div>
              <label className="text-white text-sm mb-3 block">Stipend/month</label>
              <div className="flex gap-3">
                <Input
                ...
                />
                <Select>
                  <SelectTrigger className="w-32 bg-gray-800 border-red-500 text-white">
            ...
            {/* Bonus Section */}
            <div>
              <label className="text-white text-sm mb-3 block">Bonus</label>
              <div className="flex gap-3">
